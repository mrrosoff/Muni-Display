...
# WMO weather code -> icon name (Dhole/weather-pixel-icons).
# Day variants; "tomorrow" forecast is daytime.
CODE_ICON = {
...
def load_xbm(path):
    """Parse XBM file -> (width, height, pixels[y][x] bool)."""
...
def fetch_forecast(ca_bundle, timeout=(5, 10)):
    params = {
    ...
    # index 1 = tomorrow
    return {
        "code": int(d["weather_code"][1]),
        "hi": round(d["temperature_2m_max"][1]),
        "lo": round(d["temperature_2m_min"][1]),
        "precip": int(d["precipitation_probability_max"][1] or 0),
